...
--------------------
### Rule Report
- [feat] Added storage path for unresolved calls in EVM. If an unresolved call’s callee address was read from storage, the call resolution pane will now display the full path to the storage variable.

### Solana
- [feat] Added support for rule splitting. The Solana Prover now supports the `--split_rules` CLI flag. This works the same as with Solidity except that the rules to be split are supplied by the `--rule` flag.

### Soroban
- [feat] Added multi-assert for Soroban smart contracts. Added support for `multi-assert` mode in the Soroban ecosystem. This can be enabled using the `-multi_assert_check` CLI flag and the behavior is the same as with Solidity.
