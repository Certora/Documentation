...
- Ease of use: Enum types can be casted to integer types in CVL

- A built-in rule for read-only reentrancy vulnerabilites
...
- Introducing `summarizeExtLibraryCallsAsNonDetPreLinking`` Prover option for easier setup of library-heavy code https://docs.certora.com/en/latest/docs/user-guide/timeouts.html#library-based-systems

#### Mutation testing
- New and easier to use `certoraMutate`
...
- Performance improvements in CVL typechecker (allows for faster job submission)
