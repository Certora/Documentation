...
- There are currently no official Gambit binaries for Linux arm machines. That means that Gambit will not be installed with `pip install certora-cli`, and must be {ref}`built from source <build-gambit-from-source>`.
