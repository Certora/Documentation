...
that the invariant holds after the execution of any `public` or `external` method, assuming
that it held before the method was executed (if it does hold, we say the method
*preserves* the invariant). By default, an invariant fill be checked for any `public` or `external` 
(non-`view`/`pure`) method in the scene - this can be further configured by {ref}`-parametric-contracts`.
`View` and `pure` methods are excluded from invariant checking as by definition they cannot change the state 
of their contract. 
