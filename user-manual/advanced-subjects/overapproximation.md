...
Imagine we have the following hoare logic snippet:
...
We have to make a choice on how we want the solver to model `mul`. The natural choice is to model it with normal integer arithmetic multiplication \(i.e. `mul` will behave exactly as you expect\). This means that the underlying solver will have to work within the restrictions of integer arithmetic multiplication to try to find a counterexample.  Ultimately the solver would prove this program correct.
...
Notice that the solver could choose many behaviors for `uninterp_mul` but _importantly_ these behaviors _include_ the behavior of `integer_arithmetic_mul`. This is what makes this an overapproximation--it considers program behavior that includes "actual" program behavior and more.
...
There is a middle ground that we can take between exactly modeling program behaviour and the above overapproximation. We can use uninterpreted functions and add axioms to them. In the above example, we let the solver decide everything about `uninterp_mul`. But it turns out we can give hints to the solver, to more closely approximate the behavior of `integer_arithmetic_mul`
